...
breed [troubled-youths troubled-youth]
breed [comm-workers comm-worker]
...
to setup-citizens
  create-citizens 200 [
    ...
    set children random-float 1 < 0.6 ; chance that one citizen has children is 60%- isn't this 37% of the population instead?
...
to setup-polices
  create-polices 2 [
    setxy random-xcor random-ycor ;change to police station after
    set job random-float 0 or 1
    set house patch-at 0 0; remmeber the home location of turtles
  ]
end

to live-a-police-day
  ask polices with [job = 1] [
    move-to work
  ]
  ; when shift ends, move to home, in between add pick up / drop off kids
  ; add if police have no job- live as citizens- or do nothing
    move-to house
  ]
end
